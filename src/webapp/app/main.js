...
PTM.Photo.reopenClass({
    url: 'photos.json'
});

PTM.PhotoListController = Em.ArrayProxy.create({
    content: [],
    selected: null,
    timerId: null,
    ...
    startSlideshow: function() {
        this.nextPhoto();
        this.set('timerId', setInterval("PTM.PhotoListController.nextPhoto()", 4000));
    },
...
PTM.SelectedPhotoController = Em.Object.create({
    contentBinding: 'Phototouch.PhotoListController.selected'
});

PTM.ThumbnailPhotoView = Em.View.extend({
    click: function(evt) {
    ...
    isSelected: function() {
        console.log(PTM.PhotoListController.get('selected') == this.get('content'));
        return PTM.PhotoListController.get('selected') == this.get('content');
    }.property('PTM.PhotoListController.selected')
});

PTM.SelectedPhotoView = Em.View.extend({
    swipeOptions: {
    ...
    swipeEnd: function(recognizer) {
        if (recognizer.swipeDirection === Em.OneGestureDirection.Left) {
            PTM.PhotoListController.nextPhoto();
        } else if (recognizer.swipeDirection === Em.OneGestureDirection.Right) {
            PTM.PhotoListController.prevPhoto();
        }
    },
